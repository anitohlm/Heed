...
|---|---|---|
| `heed-advisor` | GPT-4o (via Azure OpenAI) | The Advisor agent — user-facing reasoning |
| `heed-keeper` | GPT-4o-mini (via Azure OpenAI) | The Memory Keeper agent and the Bing sanitizer |
...
### Advisor — GPT-4o, selected for reasoning depth
...
### Memory Keeper — GPT-4o-mini, selected for cost and reproducibility
...
- Background runs at scale require cost discipline; GPT-4o-mini is approximately 1/15th the per-token cost of GPT-4o.
...
### Bing sanitizer — GPT-4o-mini, selected for reliability under adversarial input

The sanitizer reads raw Bing results and extracts only factual content, discarding anything resembling instructions or manipulation. GPT-4o-mini is used here intentionally: a smaller model is *less* easily redirected by indirect injection because it has less natural-language flexibility to be argued out of its system prompt.
...
Both GPT-4o and GPT-4o-mini were tested on the same Memory Keeper input — a "Call Mom" task with 22 weeks of completion data, 19 of which were Sunday completions, with three recent Sunday misses lacking a context-window explanation.
...
- [ ] Average tokens per response (4o vs 4o-mini)
- [ ] Average latency (4o vs 4o-mini)
...
Expected outcome based on prior work: GPT-4o-mini will be cheaper, faster, and stable enough for this task. The expected ship decision is GPT-4o-mini for the Memory Keeper, with GPT-4o reserved for the Advisor.
...
- [ ] Run the query through GPT-4o (current Advisor)
- [ ] Run the same query through GPT-4o-mini
- [ ] Document whether mini completed the multi-step plan, called the right tools, and produced a coherent "lighter version"
- [ ] Document specifically what mini got wrong (if anything) — this is the empirical justification for using GPT-4o on the Advisor

Expected outcome: GPT-4o-mini will partial-pass — handling simple queries cleanly but stumbling on multi-step planning. The stumble is the evidence that the larger model is the correct choice for the Advisor.
...
3. **Cost-aware deployment.** A naive design would use GPT-4o for everything. By routing simple structured tasks to mini, Heed runs approximately 10x cheaper at scale than a one-model-fits-all design.
